#!/usr/bin/env python3
"""Main drone (id=0) — leader. All drones converge on same world points
at staggered times. Position feedback ensures real physical separation."""
...
async def led_pattern(drone, shutdown):
    masks = ['1000', '0100', '0010', '0001']
    idx = 0
    while not shutdown.is_set():
        drone.set_leds(masks[idx % len(masks)])
        idx += 1
...
async def main():
    ...
    print('Connecting main drone...')
    try:
        await drone.connect()
        print('  Drone 0: connected')
    except Exception as e:
        print(f'  Drone 0: connect failed: {e}')
        rclpy.shutdown()
        return

    wpts_all = world_enu_waypoints()
    wpts = ned_for_drone(0, wpts_all)

    led_task = asyncio.create_task(led_pattern(drone, shutdown_async))
    mission_task = asyncio.create_task(
        mission(drone, 0, stop_event, shutdown_async, wpts))
    await mission_task
